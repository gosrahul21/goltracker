...
export const scheduleDailyBriefs = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!user || !user.telegramId || !user.morningBriefTime) {
      return; // Stop function if no config
    }

    // 2. Calculate the next exact timestamp for their morning brief
    const [hours, minutes] = user.morningBriefTime.split(':').map(Number);
    ...
    await step.sleepUntil("wait-for-morning", nextBrief);

    // 4. Send the brief
    await step.run("send-morning-brief", async () => {
      ...
      let text = `🌅 <b>Good Morning, ${userData.username}!</b>\n\nHere is your active goal summary:\n\n`;
    ...
    await step.sendEvent("trigger-next-day", {
